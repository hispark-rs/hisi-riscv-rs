...
//!     *name* is `hisi-riscv-link.x` for BOTH chips: hisi-riscv-rt's build.rs always
//!     writes `hisi-riscv-link.x` (it only varies what it `INCLUDE`s — WS63 device.x
//!     vs the PAC-supplied BS21 device.x, and the boot-header only when its
//!     feature is on). So this single `-Thisi-riscv-link.x` flag is chip-agnostic and
...
//!     examples/bs21/*/build.rs, which both use `-Thisi-riscv-link.x`).
//!
...
//! does not yet ship a BS21 `memory.x`, so `hisi-riscv-link.x`'s `INCLUDE memory.x`
//! will not resolve for BS21 until a BS21 board + memory.x are added here (see
//! examples/bs21/clock_rng for the pattern).
fn main() {
    // Entry script name is the same (`hisi-riscv-link.x`) for WS63 and BS21 — rt only

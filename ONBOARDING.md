...
- [ ] hisi-riscv-rust-toolchain — https://github.com/hispark-rs/hisi-riscv-rust-toolchain (official Rust nightly radar for `riscv32imfc-unknown-none-elf`; no longer the default custom rustc install path)
...
- [ ] /run-ws63-rs — build / check / clippy / fmt the workspace with the pinned official Rust nightly (start here to confirm your toolchain works)
...
- **Submodules are everything.** Core submodules: `crates/pac/{ws63-pac,bs2x-pac}` (PACs), `crates/{hisi-hal,hisi-riscv-rt,ws63-radio-sys}` (HAL, runtime, radio ABI/link integration), `examples/ws63` (chip examples), and `chips/{ws63,bs2x}/guide` (hardware guides). The language-neutral `ws63-RF` payload is nested under `ws63-radio-sys`; SVD inputs are nested under their PAC repositories. Always clone/update with `git submodule update --init --recursive`.

...
- **Submodules are everything.** Core submodules: `crates/chips/ws63/{ws63-pac,ws63-radio-sys,hisi-crypto-ws63}`, `crates/chips/bs2x/bs2x-pac`, `crates/{hisi-hal,hisi-riscv-rt}` (chip-neutral HAL/runtime), `examples/ws63`, and `chips/{ws63,bs2x}/guide`. The language-neutral `ws63-RF` payload is nested under `ws63-radio-sys`; SVD inputs are nested under their PAC repositories. Always clone/update with `git submodule update --init --recursive`.

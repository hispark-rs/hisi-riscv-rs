...
- **Validity invariant（有效性不变式）**：必须**始终**成立（连 unsafe 内部都不能违反），编译器依赖它做优化，违反 = UB。例：`&mut T` 指向的内存必须是对齐、非空、可解引用且包含合法 `T` 值。
- **Safety invariant（安全不变式）**：模块对外承诺的额外不变式，可能比 validity 更强。模块内部可以用 raw pointer、`MaybeUninit`、私有状态机等方式暂时处在"safe 调用者不可观察"的中间状态，但不能创建违反 validity 的已类型化 Rust 值；在回到 safe API 边界前必须恢复这些安全不变式。例：`Vec` 可以持有未初始化 spare capacity，但不会把它作为 `&T` 暴露给 safe 代码。

**对 HAL 的含义**：`PeripheralTransfer` 拥有 DMA 缓冲区。DMA 传输期间，safe 调用者无法访问该缓冲区；DMA 引擎和 CPU cache 之间的可见性由 guard 管理。`wait()` 完成后做方向正确的 cache maintenance（RX invalidate / TX clean 已在启动前完成），并在超时路径先 quiesce 硬件再释放资源。这里要恢复的是"缓冲区不再被硬件访问、CPU 看到的数据是同步后的"安全不变式，而不是允许 Rust validity 被暂时破坏。
...
- 方向正确：大多数 unsafe 被封装在驱动模块内部，公共 API 尽量通过 typed config、所有权 token、borrow checker 暴露。
...
- 当前缺口：P0 还未完成。以 `cargo clippy --no-default-features --features chip-ws63 --target x86_64-unknown-linux-gnu -- -W clippy::undocumented_unsafe_blocks` 检查时，`hisi-riscv-hal` 仍有大量 undocumented unsafe warnings。因此下面的 P0 是 release 前目标，不是当前已满足状态。
...
#### Unsafe 封装自动审计研究（Safe4U 等，待复核）

- 方向：用静态分析 + LLM 辅助识别 safe API 内部封装 unsafe 时遗漏的前置条件。
- 对本仓意义：可以作为人工 unsafe audit 的候选生成器，帮助列出"safe 参数是否未经验证进入 unsafe"的审查队列。
- 当前限制：本报告中的具体 Safe4U 数字和 DOI 需要二次复核；在工具链可安装、数据集可复现、误报率可量化前，不作为 release gate。
...
#### Harness 生成研究（HarnessLLM 等，待复核）

- 方向：用 LLM 生成测试或验证 harness，扩大 Miri/Kani/单元测试的路径覆盖。
- 对本仓意义：可以辅助生成 host-testable 纯逻辑函数的测试草稿，例如 DMA length/bounds、typed config 构造器、timeout 状态机。
- 当前限制：具体论文、指标和 Rust-verification 适配度需要复核；生成的 harness 必须由人工审阅，并由 Miri/Kani/测试实际执行结果背书。
...
| AI 辅助 unsafe 审计 | 低-中 | 分钟到小时 | 候选生成，需人工确认 |
...
| **注释审计** | SAFETY 注释 + code review | 低 | unsafe 块级 | LLM 辅助生成审查候选，人工确认 |
| **静态分析** | clippy + cargo-deny | 低 | 全局 | **LLM 驱动符号执行发现越界路径** |
| **动态测试** | unit + property tests + Miri | 中 | 代码路径 | LLM 辅助生成 harness 草稿 |
...
|---|---|
| **P0**: SAFETY 注释 | ✅ 继续。先用 clippy 建基线，再人工补齐高风险模块 |
| **P1**: Typestate 扩展 | ✅ 继续。不变 |
| **P2**: Kani 试验 | ✅ 先手工挑选纯逻辑函数并写最小 harness；AI 可辅助生成草稿，但不能替代审阅 |
| **P3**: Miri 试验 | ✅ 继续。LLM 可辅助生成测试场景草稿 |
| **P5**: 关注别名模型 | ✅ 继续。不变 |
| **新 P-AI1**: AI 辅助审计 spike | 实验性：用 LLM 辅助梳理 unsafe→safe 封装，但发现必须由人工确认 |
| **新 P-AI2**: AI Kani/Verus spike | 实验性：对关键纯逻辑函数生成 harness / spec 草稿，作为人工审阅输入 |
| **新 P-AI3**: Rust→Lean spike | 实验性：评估 Charon→Aeneas 对 DMA 核心代码的可翻译范围，不作为 release gate |
...
>
> → **2026 年更新**：AI 辅助的自动化验证（AutoVerus/KVerus/VeriStruct 等）已经显著降低了 proof / harness 生成成本。对生产 HAL 来说，它们值得进入 **experimental CI / nightly audit**，但在本仓库尚未有 pinned 工具链、可复现安装步骤、稳定误报率和硬件 stub 策略前，不应作为阻塞 release gate。RustBelt 级别的全 soundness 证明仍不必要；函数级/module 级验证可以逐步试点。
>
> **务实判断**：AI 让验证从"完全不现实"变成"值得工程化试点"，但真正的门禁仍必须依赖可复现工具输出（clippy/Miri/Kani/Verus/Lean kernel）和人工审阅，而不是 LLM 的自然语言结论。

#### 一条务实的集成路径

```
PR 提交 → 阻塞 CI：
  1. cargo check / clippy / fmt / tests
  2. -W clippy::undocumented_unsafe_blocks（先记录基线，再逐步收敛）
  3. 关键 host-testable 逻辑的 unit/property tests

Nightly / 手动验证：
  4. Miri（仅 host-testable 路径）
  5. Kani/Verus spike（仅已 stub 的纯逻辑/状态机函数）
  6. AI 辅助审计输出 → 人工确认 → 转成 issue 或修复 PR
```

这条路径把"可复现、低误报、能阻塞 PR"和"研究工具试点"分开。AutoVerus/KVerus 的论文结果说明方向值得投入，但 HAL 的 MMIO、DMA、cache、interrupt 语义仍需要本仓库自己的 stub 和 harness 设计。
...
多数 proof-generation 工具（AutoVerus、KVerus、AlphaProof、Lean Copilot）的架构都是：
> LLM 生成候选证明 → 形式验证器（Lean/Verus/Z3）**严格检查** → 通过才接受

与通用 AI 生成代码不同，已被验证器接受的证明不会只停留在"看起来对"。但风险没有消失：规约可能写错、stub 可能漏掉硬件前提、验证目标可能过窄，或者工具链不可复现。因此 AI 输出只能作为候选，最终可信对象是 pinned 工具链检查过的 proof/harness/spec。

#### 3. 别名模型风险是否被 AI 验证覆盖？→ **部分**

Tree Borrows 为 SOTA 别名模型。Miri 已经实现了 Tree Borrows 检查。如果 Rust 官方采纳 Tree Borrows，Miri 会立即检出违规代码。AI 验证工具（如 AutoVerus）本身不直接验证别名合规性；Miri 是动态的，不能穷尽。LLM 生成的 test harness 草稿可以扩大路径覆盖，但必须由 Miri/测试执行结果确认。

#### 4. 谁来做集成工作？→ **需要一位"验证工程师"**

AI 工具降低了 proof/harness 草稿生成成本，但仍需要有人配置 CI 流水线、编写 stub、决定哪些函数需要验证。这不是 PhD 级别的工作，但也需要理解 Rust unsafe 和形式化验证的基础概念。对小型团队（如 hisi-riscv-rs），建议先做"LLM 辅助 unsafe 审计候选 + 人工确认"这一最小可行步骤。
...
|---|---|---|
| Rust soundness 证明 | RustBelt：3 年 + 4 PhD + 14,000 行 Coq | AI/Verus 工具可辅助函数级证明，但不替代类型系统级 soundness 证明 |
| Unsafe 封装审计 | 手工 code review | LLM/静态分析可辅助生成候选，最终仍需人工确认 |
...
| 对所有 HAL 的实操建议 | 跳过全栈形式化证明（ROI 不匹配） | AI 辅助验证先做 experimental CI / nightly audit，成熟后再提升为门禁 |

**一句话总结**：AI 没有直接降低 RustBelt 的成本（RustBelt 的类型系统级 soundness 证明仍需要 PhD 级工作），但它让一部分函数级、模块级验证任务从"不现实"变成了"值得试点"。对于生产嵌入式 HAL，"全栈形式化证明"仍然过重；当前务实路径是先把 SAFETY 注释、clippy、host tests、Miri/Kani 小范围 harness 做扎实，再把 AI 辅助验证作为可复现工具链之上的加速层。
...
- Safe4U / unsafe encapsulation audit line: **待复核**（本报告不再把其具体数字作为工程依据）
...
- Harness generation line: **待复核**（可作为研究方向，不作为 release gate 依据）

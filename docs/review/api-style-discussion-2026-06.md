...
| Keep `chip-ws63` in `default`? | esp-hal ships **no** default chip (a BS21 user who forgets `--no-default-features` gets a *loud* `compile_error`, not a silent WS63 build) vs ergonomics for the dominant WS63 audience (`cargo build`/`doc` "just works") | **Keep `chip-ws63` default for now** (WS63 is the north star, BS21 a later milestone) but **document the footgun explicitly**. Revisit if BS21 adoption grows. |

...
- **异步（超出原评审范围，已实现）**：hisi-riscv-hal 新增 `async`/`embassy` —— `embedded-hal-async`/`embedded-io-async` 全套（DelayNs/Wait/SpiBus/I2c/Read/Write）+ embassy-time `Driver` + embassy-executor 多任务，全部 ws63-qemu 验证（见 [async-embassy.md](../architecture/async-embassy.md)）。
...
- HAL 核心与驱动：[../architecture/hisi-riscv-hal.md](../architecture/hisi-riscv-hal.md)
- 运行时与 bootloader：[../architecture/hisi-riscv-rt.md](../architecture/hisi-riscv-rt.md) · [../architecture/ws63-flashboot.md](../architecture/ws63-flashboot.md)

...
- **异步（超出原评审范围，已实现）**：hisi-riscv-hal 新增 `async`/`embassy` —— `embedded-hal-async`/`embedded-io-async` 全套（DelayNs/Wait/SpiBus/I2c/Read/Write）+ embassy-time `Driver` + embassy-executor 多任务，全部 ws63-qemu 验证（见 [async-embassy.md](../src/explanation/components/06-async-embassy.md)）。
...
- 逆向文档书：[../src/explanation/components/10-ws63-guide.md](../src/explanation/components/10-ws63-guide.md)

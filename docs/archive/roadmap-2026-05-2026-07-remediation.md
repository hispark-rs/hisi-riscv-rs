# Archived ROADMAP: 2026-05 to 2026-07 remediation ledger

> This file preserves the original remediation roadmap and discovery ledger from
> the 2026-05 architecture review through the 2026-07 stabilization work. It is
> historical context, not the current priority order.
>
> Current planning lives in [`../../ROADMAP.md`](../../ROADMAP.md). Use that file
> for active milestones and use this archive only when tracing why earlier
> build, HIL, runtime, HAL, RF, or documentation decisions were made.

# ws63-rs 路线图（ROADMAP）

> 本路线图源于 2026-05 的一次深度架构评审（多 agent 工作流 + 对抗式验证，41 条发现 0 条被驳回）。
> 完整发现台账见 [`docs/review/architecture-review-2026-05.md`](../review/architecture-review-2026-05.md)，
> 各组件架构与评审见 [`docs/src/explanation/components/`](../src/explanation/components/)。
>
> **API 设计评审（2026-06）**：HAL API 风格对照业界（esp-hal / embassy / embedded-hal 1.0 /
> Rust API Guidelines）的讨论台账见 [`docs/review/api-style-discussion-2026-06.md`](../review/api-style-discussion-2026-06.md)；
> 头号约定「类型化配置：能编译就能在硅片上跑」见 [`docs/src/explanation/policies/01-typed-config.md`](../src/explanation/policies/01-typed-config.md)
> + `CLAUDE.md`「Typed config」+ `.agents/skills/typed-config/`。这些喂入 0.5.0 以来的 config 收紧（见阶段 7）。
...
| 1 | 硬件在环（HIL）bring-up + 链接脚本集成 | ✅ 驱动级已完成并持续扩展：链接脚本 ✅、QEMU 软件在环 ✅、**probe-rs download 为一等公民烧录路径**（HW-validated，blinky 物理点灯）、**WS63 HAL embedded-test HIL 已在硅片通过**；精确 stable API 证据源见 `docs/src/reference/10-stable-api.md`。示例 smoke 与连接性上板属阶段 4/5 |
...
| 6 | async（embassy） | ✅ 已完成（async HAL + embassy 时间驱动 + 6 示例，见 [docs/src/explanation/components/06-async-embassy.md](../src/explanation/components/06-async-embassy.md)；支持 chip-ws63/chip-bs21 特性） |
...
> **当前焦点**：阶段 0/1/2/3/6/7 已收口——WS63 + BS2X（BS21/BS20）QEMU 软件在环成熟、
> **真机驱动级 bring-up 完成并扩展到 0.6.0 stable API 审计线**（probe-rs download 烧录 + WS63 HAL embedded-test
> HIL 已在硅片通过；精确覆盖见 `docs/src/reference/10-stable-api.md`）；早期库栈已**硅验证发布**到 crates.io（ws63-pac 0.2.0 / rt 0.2.2 / hal 0.4.0，2026-06-15）。
> 下一步主攻**北极星——连接性上板**：阶段 4（`ws63-rf-rs` blob 链接 + pbuf/TX-sink pin）→ 阶段 5
> （scan→connect→ping），阶段 1 已就绪不再是阻塞。
> 注：许多组件文档（如 overview.md）仍聚焦 WS63；BS2X 多芯片文档覆盖见 bs2x-guide，示例成员事实见 `docs/src/reference/02-examples.md`。
...
- **ISA 修正**：先用 builtin 无原子 `riscv32imc`（软浮点、stable）做过渡，随后**切到自定义 `ws63` 工具链**——
  ...
  **无需 `-Z build-std`**。`portable-atomic` 用 `critical-section` polyfill，`hisi-riscv-rt` 的 `riscv` 开
  ...
  - **工具链补全（2026-06）**：sysroot 此前缺件，现已齐——`rust-analyzer-proc-macro-srv`（修复 LSP「cannot find
    ...
    发布 **v1.96.0-1**（rustc 仍 1.96.0，纯打包热修；旧 v1.96.0 保留）。
...
>     新增 `examples/ws63/dma_loopback`：mem↔SPI0 外设 DMA 环回（MDMA ch0，fc=1/2，握手 7/8）+ SDMA
...
先消除最大未知：写最小 crate 链接 `chips/ws63/rf/ws63-RF/lib/libwifi_rom_data.a`（仅 3KB）并解析其外部符号，
**证明工具链/链接路径走得通**。ABI 已就绪：仓库已用 `ws63` 硬浮点工具链（`riscv32imfc`/`ilp32f`），与 blob ABI 一致。

**已交付**：`examples/ws63/wifi_blob_link` 用 `--whole-archive` **全量**静态链接该 blob（确认其 `rv32imfc`/`ilp32f`
...
实现 `chips/ws63/rf/ws63-RF/include/port/` 的最小桩（`port_log`/`port_osal`/`port_oal`）+ 与 Wi-Fi/BT 协处理器的
...
（`chips/ws63/rf/ws63-RF/rom/ws63_acore_rom.lds`，3752 符号）+ compiler-rt **可重定位链接成单一对象，0 个重复符号**；从
...
完成符号解析**。可复现：`chips/ws63/rf/tools/mac-link-residual.sh`。
...
3. ✅ **WS63 HAL embedded-test 套件已在硅片通过并持续扩展**：具体用例数、覆盖面与 stable/unstable 边界以
   [`docs/src/reference/10-stable-api.md`](../src/reference/10-stable-api.md) 为唯一事实源。
4. ✅ **首板验证目标达成**：时钟修复与默认稳定驱动子集已在真硅片核实。**阶段 4/5（连接性上板）随之解锁。**

> 增量项（非阻塞）：示例级 smoke、需要外部台架的真实波形/总线业务事务、RTC 晶振路径、连接性 HIL 继续推进。
...
[`docs/review/architecture-review-2026-05.md`](../review/architecture-review-2026-05.md)。

另有一份 **2026-06 API 风格评审台账** [`docs/review/api-style-discussion-2026-06.md`](../review/api-style-discussion-2026-06.md)
...
| 高 | 从未上板验证 / 测试恒真 | ✅ 已破：**上板验证完成并持续扩展**（真实 WS63 硅片）——probe-rs download 烧录 + WS63 HAL embedded-test HIL；精确覆盖见 `docs/src/reference/10-stable-api.md`。host 侧真单测/property（`ci.yml`）+ ws63-qemu 软件在环冒烟。剩示例 smoke、外部台架场景、连接性 HIL 为增量项 |

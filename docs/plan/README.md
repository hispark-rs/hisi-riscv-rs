...
|---|---|---:|---|---|
| [Connectivity 全栈](hisi-connectivity-stack.md) | 执行中 | P0 | A5B bounded start/cancel、host key conservation 与 A5F facade 已闭合；release-train target response bound、真实 control/connect failure injection 和 fail-closed QEMU/HIL 仍未闭合；pure WPA3 另为外部阻塞门槛 | 保持现有回归和 oracle，先关闭不依赖 AP 的 source-path failure injection，再补目标端 acceptance；全部门槛闭合前不切默认 backend |

...
|---|---|---:|---|---|
| [Connectivity 全栈](hisi-connectivity-stack.md) | 执行中 | P0 | A5B bounded start/cancel、host key conservation、真实 control/connect failure injection 与 A5F facade 已闭合；release-train target response bound 和完整 fail-closed QEMU/HIL 仍未闭合；pure WPA3 另为外部阻塞门槛 | 保持现有回归和 oracle，先重跑 released state machine 的目标端响应上界，再闭合统一 parser/final-image acceptance；全部门槛闭合前不切默认 backend |

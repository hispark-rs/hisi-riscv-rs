# `cargo-hisi` 开发工作流 CLI 计划

**状态：**延期。等待 WS63 普通 `cargo build` 和单依赖 `hisi-rf` 路径完成证明。本计划
属于独立工具轨道，不阻塞当前 WPA3/connectivity 里程碑。
...
- GitHub 仓库：`hispark-rs/cargo-hisi`。
...
### `cargo hisi flash` / `monitor`：烧录与监控
...
- Host 单元测试：metadata 合并、配置优先级、产物选择、错误映射、锁、脱敏、路径处理和
  timeout/cancel 状态机。
...
- 委托一致性：比较 Cargo 直接构建与 `cargo hisi build` 的 ELF；比较直接调用
  `hisi-fwpkg` 与 CLI 生成的镜像字节；

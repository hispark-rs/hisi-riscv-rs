...
**门槛：**fixture 可由一条 hermetic host 命令重建，hash 稳定，reader 对损坏变体的错误
分类有对抗测试。
...
**门槛：**至少一个真实官方 fixture byte parity + 一个 Rust image 真机 HIL。仅“自己的
...
**门槛：**CLI golden test、malformed image fuzz/property test，以及 `hisi-fwpkg` package
roundtrip 不改变 payload bytes。

### N4 -- Factory、Backup 与升级
...
- Host：官方 fixture、roundtrip、非法 bounds/CRC/complement、重复 sequence、确定性构建和
  语义 diff。

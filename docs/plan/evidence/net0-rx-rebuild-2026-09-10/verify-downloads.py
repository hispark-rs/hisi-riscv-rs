...
# dependencies = []
# ///
"""Verify downloaded NET0 descriptor rebuild consumer reports for exact source CI."""
import argparse
import hashlib
import io
import json
...
def main():
    ...
    for host in ("ubuntu-latest", "macos-14", "windows-latest"):
        ...
        assert rx["schema"] == "net0-rx-stop-link/v2" and rx["rejected_call_and_address_mutations"] == 25
        assert len(rx["edges"]) == 18 and len(rx["veneers"]) == 7 and rx["metadata"]["bytes"] == 68
    ...
    result = {"schema": "net0-rx-rebuild-downloads/v1", "source": a.source, "ci_url": ci["url"],

...
p.add_argument("--require-host-tx", action="store_true")
p.add_argument("--require-rx-stop", action="store_true")
p.add_argument("--require-rx-rebuild", action="store_true")
p.add_argument("--reset-peer", help="Reset the AP before every STA round and require its ready marker")
a = p.parse_args()
if a.require_rx_rebuild and not a.require_rx_stop:
    p.error("--require-rx-rebuild requires --require-rx-stop")
...
for index in range(1, a.runs + 1):
    ...
    if a.require_host_tx:
        fields = ["accepted", "processed", "dropped", "pending", "peak", "rejected", "callback_errors", "closed", "fault"]
        samples = re.findall(rb"(?m)^RFDBG_NET0_HOST_TX" + rb" 0x([0-9a-fA-F]{8})" * len(fields) + rb"\r?\n", target)
        decoded = [dict(zip(fields, [int(value, 16) for value in sample])) for sample in samples]
        valid = len(decoded) == 2
        for sample in decoded:
            valid &= sample["accepted"] == sample["processed"] + sample["dropped"] + sample["pending"]
            valid &= sample["fault"] == 0 and sample["callback_errors"] == 0
        if decoded:
            final = decoded[-1]
            valid &= final["accepted"] > 0 and final["pending"] == 0 and final["closed"] == 1
        run["host_tx"] = {"samples": decoded, "pass": bool(valid)}
        run["pass"] &= bool(valid)
    if a.require_rx_stop:
        fields = ["installed", "requested", "entered", "returned", "post_returned", "mac_before", "mac_after", "descriptors_empty", "native_status", "fault", "expected_task", "current_task"]
        samples = re.findall(rb"(?m)^RFDBG_NET0_RX_STOP" + rb" 0x([0-9a-fA-F]{8})" * len(fields) + rb"\r?\n", target)
        decoded = [dict(zip(fields, [int(value, 16) for value in sample])) for sample in samples]
        valid = len(decoded) == 2
        if valid:
            final = decoded[-1]
            valid &= all(final[key] == 1 for key in fields[:5])
            valid &= final["mac_after"] == 0 and final["descriptors_empty"] == 1
            valid &= final["native_status"] == 0 and final["fault"] == 0
            valid &= final["expected_task"] not in (0, 0xffffffff) and final["expected_task"] == final["current_task"]
        run["rx_stop"] = {"samples": decoded, "pass": bool(valid),
                          "boundary": "Immediate device stop receipt; not reusable DMA/host-queue fence"}
        run["pass"] &= bool(valid)
    if a.expect_cleanup_fault:
    ...
    if a.require_rx_rebuild:
        ...
        matches = re.findall(rb"(?m)^RFDBG_NET0_RX_REBUILD" + rb" 0x([0-9a-fA-F]{8})" * len(fields) + rb"\r?\n", target)
        samples = [dict(zip(fields, [int(value, 16) for value in sample])) for sample in matches]
        valid = len(samples) == 2
        if valid:
            before, final = samples
            valid &= before["attempted"] == 0 and before["cleanup_attempted"] == 0
            valid &= final["attempted"] == 1 and final["cleanup_attempted"] == 1
            valid &= sum(final["expected_" + name] for name in ["normal", "high", "small"]) > 0
            for name in ["normal", "high", "small"]:
                valid &= final["expected_" + name] == final["actual_" + name]
                valid &= final["cleaned_" + name] == 0
            valid &= all(final[name] == 0 for name in fields[11:])
        run["rx_rebuild"] = {"samples": samples, "pass": bool(valid),
                             "boundary": "Closed-admission allocation round-trip, not DMA fence or reconnect"}
        run["pass"] &= bool(valid)

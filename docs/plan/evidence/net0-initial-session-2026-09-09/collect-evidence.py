...
def summarize_case(case, output):
    ...
    for run in original["runs"]:
        ...
        classification = ("initial-session-payload-pass" if good else
        ...
        row = dict(run, classification=classification, public_excerpts=excerpts,
...
def main():
    ...
    report = {"schema": "net0-initial-session-evidence/v1", "status": "experimental-bring-up",
        "boundary": "Initial-session ARP/UDP only; native drainage, STA-only reset, reconnect, Embassy Net and HTTPS remain unaccepted",

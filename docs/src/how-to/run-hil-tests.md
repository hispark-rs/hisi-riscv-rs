...
`hil/hil-smoke.sh` 把每个示例逐个烧到真机、读串口、断言它打印了预期的标记串。它验证 QEMU 证明不了的部分——**真实时钟/时序、真实外设**（尤其是修正后的 24 MHz TCXO 定时器和 160 MHz UART 波特基准）。HIL 框架背景见[HIL 测试框架](../explanation/components/index.md)，全部标记串见[HIL 标记串与环境变量](../reference/hil-markers.md)。

> 前提：板子接好、UART0 接到 host、烧录环境就绪（见[用 probe-rs 烧录](flash-probe-rs.md)，hil-smoke 默认通过 `hil/flash.sh` 烧录，即默认 `METHOD=probe-rs`）。

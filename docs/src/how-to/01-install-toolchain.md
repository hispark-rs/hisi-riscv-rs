...
工具链的来历、ABI 取舍见[硬浮点工具链](../explanation/03-hardfloat-toolchain.md)；target 命名细节见[工具链与编译目标](../reference/05-toolchain.md)。
...
确认 `core` 预编译可用（直接试构建，下一篇[如何构建一个示例](02-build-example.md)）：

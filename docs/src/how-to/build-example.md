...
仓库带了一批 WS63 示例（`blinky`、`uart_hello`、`timer_irq`、`spi_loopback`、`i2c_scan`、`async_delay`、`embassy_multitask`…，完整清单见[示例目录与验证标记串](../reference/examples.md)）。本篇讲怎么把它们编出来。

> 前提：已[安装 hisi-riscv 工具链](install-toolchain.md)。在仓库目录里 `rust-toolchain.toml` 会自动选它，默认 target 是 `riscv32imfc-unknown-none-elf`，无需 `+hisi-riscv` 或 `--target`。
...
打包成可启动镜像时 `hisi-fwpkg` 直接吃 ELF（见[如何打包镜像](package-image.md)），**通常不需要**手动 objcopy。但若某条工具链确实要裸 bin，用工具链自带的 `rust-objcopy`：

...
- **WS63 未测试驱动**：`clock_init`/`shared_memory`/`km`/`safety`/`sfc`/`ulp_gpio`/`rtc`-WS63（`hil-rtc` 是 opt-in 且这块板没晶振从没跑过）/`delay`。`shared_memory` 当前只服务 RF bring-up，尚未作为通用 HAL 能力毕业。`Spacc`/`Pke` 仅保留 peripheral token，不存在 HAL 算法 driver；算法能力归芯片 crypto backend。

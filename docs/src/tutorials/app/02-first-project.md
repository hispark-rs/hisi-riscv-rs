...
2. `hisi-fwpkg plan --image-output` 生成完整 flash image 和 `*.plan.json`；
3. 从 plan 读取 `base_addr`；
4. 用打补丁的 probe-rs 分支执行 `download --binary-format bin --base-address <plan.base_addr> <image>`；
5. `probe-rs reset` 复位，flashboot 跳进应用入口（`app + 0x300`）。
...
> **真实 body SHA-256**（这正是 `hisi-fwpkg plan` 在做的事），而不存在一个能让它启动的"占位签名"。
> 镜像格式细节见 [应用镜像格式与签名](../../reference/06-image-format.md)。
>
> `patch-hash` 只用于 `probe-rs run` / embedded-test 这类需要 ELF 符号和半主机元数据的路径；普通烧录走 `plan`。
>

...
[async-embassy 深入文档](components/async-embassy.md)；这里建立的是全局直觉。
...
（详见 [硬浮点工具链](hardfloat-toolchain.md)）。而异步执行器、waker 这些东西通常被认为
"当然要原子操作"。它怎么还能跑？
...
这里有个和 [HIL 框架](hil-framework.md) 直接相关的细节：**时间的真值来自 TCXO（24 MHz），
...
[async-embassy 深入文档](components/async-embassy.md) 里。那篇是权威；本篇负责让你先有
全局图景。

...
当前 bring-up 状态。操作步骤见 [运行 HIL 冒烟测试](../how-to/run-hil-tests.md)，
标记串与环境变量见 [HIL 标记串与环境变量](../reference/hil-markers.md)；本篇讲"为什么这样测"。

## HIL 存在的意义：验 QEMU 验不了的那部分

[QEMU](qemu-model.md) 已经把逻辑钉死了——内存布局、启动序列、外设行为、中断投递、DMA 搬运

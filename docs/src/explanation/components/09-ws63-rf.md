...
`ws63-RF` 是嵌套在 `crates/chips/ws63/ws63-radio-sys/` 下的语言中立 git submodule，定位是**连接性（Wi-Fi/BT/BLE/SLE）的 vendor payload**。`ws63-radio-sys` 固定其版本并拥有 archive/ABI/post-link contract。payload 本身负责两件事：
...
它**不负责**：
- 不提供 Rust 绑定或 Cargo package；这些由 `ws63-radio-sys` 提供，payload 自身仍对 Cargo workspace 不可见。
...
子模块内自带 `ARCHITECTURE.md`。其中早期“连接性 0% / 尚无 Rust 绑定”的结论已经
过时：`ws63-rf-rs` 已实现 WS63 porting/ABI adapter，调度与 IPC 已下沉到
`hisi-rf-rtos-driver` + `hisi-rtos`，芯片中立控制面位于独立 `hisi-rf`。完整 init closure
由 stock-`rust-lld` layout 驱动的受控 relocation patch lane 生成，并在 final link 后校验
section 地址；原厂 linker 只保留为诊断 oracle。真实 WS63 EVB 已完成主动扫描、
WPA2-Personal 关联、DHCP、smoltcp neighbor discovery、公共 ICMP 和 DHCP renew。

## 在依赖链中的位置

ws63-RF（blob）先经 **`ws63-radio-sys` / `hisi-rf-link`** 固定 ABI、archive profile 与
relocation；过渡 crate **`ws63-rf-rs`** 实现芯片 adapter，`hisi-rf` 提供用户控制面和
L2 device，IP stack 由应用选择 smoltcp 或 Embassy Net：

```mermaid
graph TD
    subgraph 主链["已接入的 Rust 主链"]
        SVD[crates/chips/ws63/ws63-pac/ws63-svd] --> PAC[ws63-pac]
        PAC --> HAL[hisi-hal]
        HAL --> EX[ws63-examples]
        RT[hisi-riscv-rt] -.启动/链接脚本.-> EX
    end
    subgraph RF["crates/chips/ws63/ws63-radio-sys/ws63-RF（vendor payload）"]
        BLOB["lib/*.a 闭源协议栈"]
        PORT["include/port/port_*.h<br/>~77 个 porting 接口契约"]
        PORT -.接口契约.-> BLOB
    end
    SYS["ws63-radio-sys + hisi-rf-link<br/>ABI / archive / relocation contract"]
    PLATFORM["ws63-rf-rs<br/>WS63 ABI / OSAL / netif adapter"]
    DRIVER["hisi-rf-rtos-driver"]
    RTOS["hisi-rtos"]
    API["hisi-rf<br/>controller / runner / L2 device"]
    NET["Application IP stack<br/>smoltcp / Embassy Net"]
    HAL --> PLATFORM
    PORT --> SYS
    SYS --> PLATFORM
    PLATFORM --> DRIVER
    DRIVER --> RTOS
    API --> PLATFORM
    NET -->|消费 L2 device| API
    PLATFORM -->|实现契约 + 链接 blob| BLOB
    BLOB -.HIL:真机连接性示例.-> EX

    classDef done fill:#d5f5d5,stroke:#2a2;
    classDef hil fill:#fff3cd,stroke:#cc9;
    class SVD,PAC,HAL,EX,RT,PORT,PLATFORM,DRIVER,RTOS,API,NET done;
    class BLOB hil;
```

- **上游**：blob 由 vendor SDK（`fbb_ws63`，参考实现在 `src/drivers/chips/ws63/porting/`）编译而来，本仓库只做重分发。
- **下游**：Wi-Fi 用户入口已经迁到 `hisi-rf` 的 controller/runner/device vertical slice；
  `wifi_init_smoke` 在真机通过 WPA2 connect、DHCP、neighbor cache、ICMP 和 renew。
  BLE/SLE/coexistence 仍是后续里程碑，不能由 Wi-Fi 证据外推。
...
- **WS63 blob**：`crates/chips/ws63/ws63-radio-sys/ws63-RF/lib/libwifi_driver_dmac.a` 等 vendor archives，覆盖 Wi-Fi MAC/RF/BLE/SLE。
...
- **hisi-hal 多芯片**：HAL standalone 无默认 chip；WS63 下游显式启用 `chip-ws63`，实验性 `chip-bs21` 还需 `unstable`。porting 层（ws63-rf-rs）当前绑 WS63，BS2X 连接性交付另行规划。

详见 [当前 ROADMAP](https://github.com/hispark-rs/hisi-riscv-rs/blob/main/ROADMAP.md)、
[A4 connectivity plan](https://github.com/hispark-rs/hisi-riscv-rs/blob/main/docs/plan/hisi-connectivity-stack.md)
与 [overview.md](01-overview.md)。

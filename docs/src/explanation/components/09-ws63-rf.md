...
子模块内自带 `ARCHITECTURE.md`。注意其"连接性 0% / 尚无 Rust 绑定"的旧结论**已过时**：in-tree crate **`ws63-rf-rs` 现已实现 porting 契约**（osal/oal/log/uapi/frw/hcc + 协作调度器 + 软件计时器 + netif→smoltcp 桥），且 **Wi-Fi-init 的 relocatable 符号闭合已达成**（whole-archive 0 重复符号，`--gc-sections` rooted at `uapi_wifi_init` 残留仅 2 个 firmware linker 符号）。`hisi-riscv-rt` 现在提供真实 `.wifi_pkt_ram (NOLOAD)`，剩余硬阻塞是完整 init closure 进入 final executable 时的 HiSilicon `R_RISCV_48_LLUI` relocation 58：原厂 linker 可处理，stock `rust-lld` 目前不能处理。

...
子模块内自带 `ARCHITECTURE.md`。注意其"连接性 0% / 尚无 Rust 绑定"的旧结论**已过时**：in-tree crate **`ws63-rf-rs` 现已实现 porting 契约**（osal/oal/log/uapi/frw/hcc + 协作调度器 + 软件计时器 + netif→smoltcp 桥）。完整 init closure 现在由 stock-`rust-lld` layout 驱动的受控 relocation patch lane 生成，并在 final link 后校验 section 地址；原厂 linker 只保留为诊断 oracle。真实 WS63 EVB 已通过 `wifi_init`，并由主动扫描返回 32 个 AP；当前边界是尚未完成关联与 IP 连通性。

## 在依赖链中的位置

ws63-RF（blob）先经 **`ws63-radio-sys` / `hisi-rf-link`** 固定 ABI、archive profile 与 relocation，再由过渡 crate **`ws63-rf-rs`** 实现 porting 契约：

...
`ws63-RF` 是嵌套在 `crates/ws63-radio-sys/` 下的语言中立 git submodule，定位是**连接性（Wi-Fi/BT/BLE/SLE）的 vendor payload**。`ws63-radio-sys` 固定其版本并拥有 archive/ABI/post-link contract。payload 本身负责两件事：
...
graph TD
    ...
    subgraph RF["crates/ws63-radio-sys/ws63-RF（vendor payload）"]
        BLOB["lib/*.a 闭源协议栈"]
...
- **WS63 blob**：`crates/ws63-radio-sys/ws63-RF/lib/libwifi_driver_dmac.a` 等 vendor archives，覆盖 Wi-Fi MAC/RF/BLE/SLE。

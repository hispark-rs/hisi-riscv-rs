...
这一节是组件/平台深入文档的索引。它们是每个组件的**权威实现说明**——讲这一个组件
内部到底怎么实现、职责边界在哪里、为什么采用当前设计。和上一层的概念章节分工明确：
...
- [hisi-hal.md](04-hisi-hal.md)——手写的安全驱动层，多芯片、可选 async/embassy。
...
- [ws63-flashboot.md](08-ws63-flashboot.md)——实验性 Rust 二级引导的架构
...
- [hi3322-runtime-porting.md](hi3322-runtime-porting.md)——Hi3322 runtime 移植预研：TES/TEE
  reset、CLIC、中断、内存布局与 `riscv-rt` 复用边界。

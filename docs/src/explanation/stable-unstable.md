...
这是本项目 HAL（0.6.0+）的**第二号约定**：凡是没有上板 HIL 真机测试覆盖的接口，都关在 `unstable` feature 门后 —— 默认 `cargo build` 只暴露经过硅片验证的稳定 API，实验性接口要用户显式 `features = ["unstable"]` 才能用。这和 [类型化配置](./typed-config.md) 互补：一个保证"能编译就能跑"，一个保证"默认暴露的都是跑过的"。

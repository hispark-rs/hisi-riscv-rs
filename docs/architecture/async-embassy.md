...
> ws63-hal 的异步层（`async`/`embassy` feature）如何工作、代码在哪、以及之后如何上游化。
> 总体架构见 [overview.md](overview.md)。

## 一句话

ws63-hal 在阻塞驱动之上加了**一层中断 + waker 驱动的异步驱动**(`async` feature) 和**一个 embassy-time
...
### 1. `asynch::block_on` + `IrqSignal`(`ws63-hal/src/asynch.rs`)
...
- ws63-hal 一直用 `portable-atomic`(开 `critical-section` feature)做 CAS 垫片;`ws63-rt` 提供 `riscv/critical-section-single-hart` 实现。
...
## embassy-time `Driver`(`ws63-hal/src/embassy.rs`)
...
| 文件 | 内容 |
|------|------|
| `ws63-hal/src/asynch.rs` | `block_on` + `IrqSignal`(地基)|
| `ws63-hal/src/embassy.rs` | embassy-time `Driver`(now/alarm/queue)|
| `ws63-hal/src/timer.rs` (末尾) | `AsyncDelay`(`DelayNs`)+ `on_interrupt` |
| `ws63-hal/src/gpio.rs` (末尾) | `Wait`(GPIO 边沿/电平)+ `on_interrupt` |
| `ws63-hal/src/uart.rs` (末尾) | `embedded_io_async::{Read,Write}` + `on_interrupt` |
| `ws63-hal/src/spi.rs` (末尾) | `embedded_hal_async::spi::SpiBus`(包装阻塞)|
| `ws63-hal/src/i2c.rs` (末尾) | `embedded_hal_async::i2c::I2c`(包装阻塞)|
| `ws63-hal/src/lsadc.rs` (末尾) | `read_async`(自研;IRQ 72)|
| `ws63-hal/src/dma.rs` (末尾) | `wait_transfer_done`(自研;IRQ 59)|
| `ws63-hal/Cargo.toml` | `async` / `embassy` feature + 可选依赖 |

**示例**(均在 ws63-qemu smoke-test 验证):
`ws63-examples/async_delay`(block_on + `DelayNs`)、`async_bus`(SPI/I2C/LSADC)、
...
4. **QEMU 模型**(ws63-qemu):把 `-M ws63` 板卡 + `-cpu ws63` 命名核 + xlinx 自定义 ISA 解码上游到 QEMU —— 这是 ws63-qemu ROADMAP 阶段 6([github.com/sanchuanhehe/ws63-qemu](https://github.com/sanchuanhehe/ws63-qemu))。与本仓异步无直接关系,但能让 CI 不依赖 fork 的 QEMU。

> 简言之:**异步/embassy 这块本身已经是「按上游约定正确实现」**,真正的上游化工作量在 ① 把 ws63-hal(含 embassy feature)发版到 crates.io、② 摆脱自定义 rustc 工具链、③ ws63-qemu 进 QEMU 主线 —— 三者互相独立。

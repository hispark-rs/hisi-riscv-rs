...
- **阶段 3（链接 blob 尖刺）** ✅ 已完成：`ws63-rf-rs/build.rs` + 链接搜索把 `lib/*.a` 喂给链接器；hisi-riscv-rt 提供 `__wifi_pkt_ram_*` defsym；**Wi-Fi-init 符号闭合达成**（残留 2）。`wifi_blob_link`/`rf_port_demo` 验证。

...
- **不负责**：任何驱动逻辑、时钟门控策略、引脚复用、外设初始化时序。这些全部上移到 `ws63-hal`。PAC 只暴露"裸寄存器 + 地址映射"，是 `unsafe` 寄存器写入的最底层封装边界。
...
ws63-pac ──► ws63-hal ──► ws63-examples/*
   │
   └──► ws63-rt（rt feature 提供 device.x 中断向量 + RISCV_RT_BASE_ISA）
```

- 上游：`ws63-svd` 的 XML 描述，经 `svd2rust v0.37.1` 一次性生成（`src/lib.rs:1` doc 注释标注版本）。
- 下游：`ws63-hal`（安全驱动）与 `ws63-rt`（启动/链接）均消费本 crate。两者通过 **registry 版本依赖** `version = "0.1"` 声明（`ws63-hal/Cargo.toml:12`、`ws63-rt/Cargo.toml:21`），在 monorepo 内由根 `Cargo.toml` 的 `[patch.crates-io]` 重定向到本地路径（`Cargo.toml:50-51`），保证全工作区只链接**单一** PAC 实例。

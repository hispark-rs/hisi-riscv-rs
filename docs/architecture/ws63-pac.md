...
- 下游：`hisi-riscv-hal`（安全驱动）与 `hisi-riscv-rt`（启动/链接）均消费本 crate。两者通过 **registry 版本依赖** `version = "0.1"` 声明（`hisi-riscv-hal/Cargo.toml:12`、`hisi-riscv-rt/Cargo.toml:21`），在 monorepo 内由根 `Cargo.toml` 的 `[patch.crates-io]` 重定向到本地路径（`Cargo.toml:50-51`），保证全工作区只链接**单一** PAC 实例。
...
| 中 | 依赖 | 曾被 `hisi-riscv-hal` 以 git 依赖引入，导致工作区出现双 PAC 实例 | `hisi-riscv-hal/Cargo.toml:12`、`Cargo.toml:45-51` | 本轮已修：改 registry 版本依赖 + 根 `[patch.crates-io]` 指向本地，`cargo tree` 仅单一 `ws63-pac` 实例 |

...
```
crates/ws63-pac/ws63-svd (CMSIS-SVD) ──svd2rust──▶ ws63-pac (寄存器访问层)
                                               │
                                               ▼
...
| `crates/ws63-pac/ws63-svd` | 嵌套 submodule（在 ws63-pac 下） | SVD 真值 + 生成工具（归 pac 所有） | [ws63-svd.md](ws63-svd.md) |

...
fn main() -> ! {
    ...
    ch.configure(
        PwmPeriod::try_from_hz(1_000).unwrap(),
        Duty::from_percent(50).unwrap(),
    );
    ch.enable();
    ch.start();
    uart.write(b"  pwm ch0 started (1 kHz, 50%)\r\n");

    if wdt_ok {
        uart.write(b"  PWM+WDT OK\r\n");
    } else {
        uart.write(b"  PWM+WDT MISMATCH\r\n");
    }

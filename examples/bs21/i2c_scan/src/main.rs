...
fn write_hex2(uart: &Uart<'_, impl UartInstance>, v: u8) {
    let hex = |n: u8| if n < 10 { b'0' + n } else { b'a' + (n - 10) };
    uart.write(&[b'0', b'x', hex(v >> 4), hex(v & 0xF)]);
}
...
fn main() -> ! {
    ...
    if count == 1 && last == EXPECTED_ADDR {
        uart.write(b"  I2C scan OK\r\n");
    } else {
        uart.write(b"  I2C scan MISMATCH\r\n");
    }

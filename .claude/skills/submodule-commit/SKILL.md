...
| ws63-svd | `main` | source of the PAC — **nested submodule at `crates/ws63-pac/ws63-svd`** |
...
2. **Commit** with a clear message; end with the Co-Authored-By trailer:
   ```bash
   git -C <sub> add -A
   git -C <sub> commit -m "<subject>" -m "Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>"
   ```
...
```bash
git add <changed-submodules> Cargo.lock <other-parent-files>   # NOT .claude/worktrees
git commit -m "chore: update submodule pointers — <what>" -m "Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>"
...
git status -s                   # clean (except .claude/worktrees/)
```
...
- **Never `git add .claude/worktrees/`** — it's transient harness state.

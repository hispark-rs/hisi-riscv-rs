...
```bash
cd <repo> && bash <path>/.claude/skills/release-train/train.sh <tag> [expected_asset_count]
```
...
cd /root/ws63-qemu        && bash /root/ws63-rs/.claude/skills/release-train/train.sh v0.4.6 7

# Custom toolchain — 4 host tarballs + 4 .sha256
cd /root/ws63-rust-toolchain && bash /root/ws63-rs/.claude/skills/release-train/train.sh v1.96.0-2 8

# A library crate — crates.io publish (no GitHub assets; watches publish.yml)
cd /root/ws63-rs/crates/hisi-riscv-hal && bash /root/ws63-rs/.claude/skills/release-train/train.sh v0.3.1
```

...
See `CLAUDE.md` "Stable / Unstable API gating" section for the authoritative split, or `docs/src/explanation/stable-unstable.md` for the full explanation. The split is audited against `tests/hil.rs` — the rule is "HIL-proven on WS63 silicon = STABLE; everything else = UNSTABLE".
